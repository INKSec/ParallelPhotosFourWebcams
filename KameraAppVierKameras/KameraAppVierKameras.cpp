...
int main()
{
    ...
    cap1.set(CAP_PROP_FRAME_HEIGHT, width);
    ...
    while (true) {
        ...
        imshow("Camera 1", myImage); //Showing the videostream. Is not needed for taking pictures
    ...
    cap.release(); // release buffer memory. When zou quit without this line, it can happen that the camerastream stays open
    //cap1.release();
    //cap2.release();
    //cap3.release();
    return 0;
